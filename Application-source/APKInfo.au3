...
Global $tempPath = @TempDir & "\APKInfo\" & @AutoItPID
...
$ShowMD5 = _readSettings("ShowMD5", '')
$ShowSHA1 = _readSettings("ShowSHA1", '')
...
$strMD5 = IniRead($IniFile, $LangSection, "MD5", "MD5")
$strSHA1 = IniRead($IniFile, $LangSection, "SHA1", "SHA1")
...
$ProgramTitle = $ProgramName & ' ' & "(" & $ProgramAlpha & ")"
...
Local $fields = 9
If $ShowMD5 <> '' Then $fields += 1
If $ShowSHA1 <> '' Then $fields += 1
...
$fullWidth += $btnIconSize + 5 - $iconSize / 2 - ($rightColumnWidth - $iconSize) / 4
...
$lblIcon = GUICtrlCreateLabel('', $rightColumnStart - $iconSize / 3 - ($rightColumnWidth - $iconSize) / 8, ($placeHeight - $iconSize) / 2, $iconSize, $iconSize)
;设置某个控件的大小调整方式;
;GUICtrlSetResizing ( 控件ID, 调整方式 )
GUICtrlSetResizing(-1, $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT + $GUI_DOCKRIGHT + $GUI_DOCKTOP)

;设置“语言环境”的label
;GUICtrlCreateLabel($strLocales & ':', $localesStart, $offsetHeight, 65, $inputHeight)
;GUICtrlSetFont(16, $Font);
;GUICtrlSetResizing(-1, $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT + $GUI_DOCKRIGHT + $GUI_DOCKTOP)
;GUICtrlSetState(-1, $globalStyle)
;Local $top = $offsetHeight + $inputHeight
;设置语言栏
;$edtLocales = GUICtrlCreateEdit('', $localesStart, $top, $localesWidth + 20, $fullHeight - 5 - $top, $editFlags + $ES_NOHIDESEL)
;GUICtrlSetFont(-1, 24, $Font);
...
;创建一个按钮控件，点击可显示不同语言环境的应用名称
;GUICtrlCreateButton ( "文本", 左侧, 顶部 [, 宽度 [, 高度 [, 样式 [, 扩展样式]]]] )
;$btnLabels = GUICtrlCreateButton('...', $inputStart - $inputHeight, $offsetHeight, $inputHeight + 5, $inputHeight)
;GUICtrlSetResizing(-1, $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT + $GUI_DOCKLEFT + $GUI_DOCKTOP)
;GUICtrlSetState(-1, $globalStyle)
;GUICtrlSetTip(-1, $strLabelInLocales)

;~ 显示“应用名称”label
$inpLabel = _makeField($strLabel, 0, $inputWidth - $iconSize / 3 - ($rightColumnWidth - $iconSize) / 2)
...
$inpVersion = _makeField($strVersion, 0, $inputWidth - 5 - $halfWidth)

;包名
$inpPkg = _makeField($strPkg, 0, $inputWidth - $iconSize / 3 - ($rightColumnWidth - $iconSize) / 2)
;系统语言
...
;~ $inpMinSDK = _makeField($strMinSDK, 0, 0)

;~ 设置“最低SDK”label
$inpMinSDKLabel = GUICtrlCreateLabel($strMinSDK, $labelStart, $offsetHeight  + $labelTop, $labelWidth, $inputHeight)
GUICtrlSetResizing(-1, $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT + $GUI_DOCKLEFT + $GUI_DOCKTOP)
GUICtrlSetBkColor(-1, $GUI_BKCOLOR_TRANSPARENT)
GUICtrlSetState(-1, $globalStyle)
;~ 设置显示SDK版本号
$inpMinSDKNum = GUICtrlCreateInput('', $inputStart, $offsetHeight, $localesWidth / 2, $inputHeight, BitOR($inputFlags, $SS_CENTER))
GUICtrlSetResizing(-1, $GUI_DOCKHEIGHT + $GUI_DOCKWIDTH + $GUI_DOCKLEFT + $GUI_DOCKTOP)
GUICtrlSetState(-1, $globalInputStyle)
;~ GUICtrlSetTip(-1, $strMinSDK)
;~ 设置显示SDK版本名称
$inpMinSDK = GUICtrlCreateInput('', $inputStart + $localesWidth / 2 + 5, $offsetHeight, $halfWidth + $halfWidth - $localesWidth / 2, $inputHeight, $inputFlags)
GUICtrlSetResizing(-1, $GUI_DOCKHEIGHT + $GUI_DOCKLEFT + $GUI_DOCKRIGHT + $GUI_DOCKTOP)
GUICtrlSetState(-1, $globalInputStyle)
GUICtrlSetTip(-1, $strMinSDK)
...
;~ 设置“目标编译SDK”label
$inpTargetSDKLabel = GUICtrlCreateLabel($strTargetCompileSDK, $labelStart, $offsetHeight  + $labelTop, $labelWidth, $inputHeight)
GUICtrlSetResizing(-1, $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT + $GUI_DOCKLEFT + $GUI_DOCKTOP)
GUICtrlSetBkColor(-1, $GUI_BKCOLOR_TRANSPARENT)
GUICtrlSetState(-1, $globalStyle)
;~ 设置显示目标编译SDK版本号
$inpTargetSDKNum = GUICtrlCreateInput('', $inputStart, $offsetHeight, $localesWidth / 2, $inputHeight, BitOR($inputFlags, $SS_CENTER))
GUICtrlSetResizing(-1, $GUI_DOCKHEIGHT + $GUI_DOCKWIDTH + $GUI_DOCKLEFT + $GUI_DOCKTOP)
GUICtrlSetState(-1, $globalInputStyle)
;~ GUICtrlSetTip(-1, $strMinSDK)
;~ 设置显示目标编译SDK版本名称
$inpTargetSDK = GUICtrlCreateInput('', $inputStart + $localesWidth / 2 + 5, $offsetHeight, $halfWidth + $halfWidth - $localesWidth / 2, $inputHeight, $inputFlags)
GUICtrlSetResizing(-1, $GUI_DOCKHEIGHT + $GUI_DOCKLEFT + $GUI_DOCKRIGHT + $GUI_DOCKTOP)
GUICtrlSetState(-1, $globalInputStyle)
GUICtrlSetTip(-1, $strTargetCompileSDK)
$offsetHeight += $fieldHeight
...
$inpScreens = _makeField($strScreens, 0, $inputWidth + $iconSize / 3 + ($rightColumnWidth - $iconSize) + 10)
...
$inpDensities = _makeField($strDensities, 0, $inputWidth + $iconSize / 3 + ($rightColumnWidth - $iconSize) + 10)
...
;设置“属性”的label
$edtFeatures = _makeField($strFeatures & @CRLF & " + : " & $strUses & @CRLF & " # : " & $strImplied & @CRLF & " - : " & $strNotRequired & @CRLF & " @ : " & $strOthers, 2, 0, 0)
...
$inpMD5 = False
If $ShowMD5 <> '' Then $inpMD5 = _makeField($strMD5, 0, $editWidth, $GUI_DOCKBOTTOM)
$inpSHA1 = False
If $ShowSHA1 <> '' Then $inpSHA1 = _makeField($strSHA1, 0, $editWidth, $GUI_DOCKBOTTOM)
...
While 1
	...
	Switch $nMsg
		...
		Case $gBtn_Rename
			...
			$height = 130
			;设置重命名弹框
			$sNewNameInput = InputBox($strRenameAPK, $strNewName, $sNewFilenameAPK, "", $width, $height + 15, $pos[0] + ($pos[2] - $width) / 2, $pos[1] + ($pos[3] - $height) / 2, $hGUI)
...
Func _makeButton($label, $icon)
	;设置button的位置
	$ret = GUICtrlCreateButton('', $btnStart - $iconSize / 2 - ($rightColumnWidth - $iconSize) / 4, $offsetHeight, $btnIconSize, $btnIconSize, $BS_BITMAP)
	GUICtrlSetResizing(-1, $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT + $GUI_DOCKRIGHT + $GUI_DOCKTOP)
...
Func _makeField($label, $edtNum = 0, $width = 0, $dock = $GUI_DOCKTOP, $btnTip = False)
	...
	Else
		...
		GUICtrlSetState(-1, $globalInputStyle)
		;设置文本框的风格
		;GUICtrlSetStyle(-1, $SS_BLACKFRAME)
		$offsetHeight += $fieldHeight
	EndIf
	Return $ret
EndFunc   ;==>_makeField
...
Func _OpenNewFile($apk, $progress = True)
	...
	$MD5 = _ReplacePlaceholders($ShowMD5)
	$SHA1 = _ReplacePlaceholders($ShowSHA1)
	...
	;~ GUICtrlSetData($inpMinSDKLabel, $strMinSDK)
	...
	GUICtrlSetData($inpMinSDKNum, $apk_MinSDK)
	GUICtrlSetData($inpMinSDK, _translateSDKLevel($apk_MinSDK))
	GUICtrlSetData($inpTargetSDKNum, $apk_TargetSDK)
	...
	If $ShowMD5 <> '' Then GUICtrlSetData($inpMD5, $MD5)
	If $ShowSHA1 <> '' Then GUICtrlSetData($inpSHA1, $SHA1)
	GUICtrlSetData($inpName, $fileAPK)
...
Func _ReplacePlaceholders($pattern)
	...
	For $i = 0 To UBound($names) - 1
		...
		$hash = $hashCache[$i]
		$out = StringReplace($out, $pll, StringUpper($hash), 0, 1)
...
Func _cleanUp()
	...
	DirRemove(@TempDir & "\APKInfo", 1) ; clean files from previous runs
...
Func _translateSDKLevel($sdk, $withNumber = True)
	...
	$ret = $tmp[0] & ' (' & $tmp[1] & ')'
	If $withNumber Then $ret = 'Android ' & $ret
	Return $ret
